...
class AFACemInterventionPolicy(CooP):
    def __init__(
        self,
        concept_group_map,
        cbm,
    ...
    def _importance_score(
        ...
        for concept_val_idx in range(len(group_concepts)):
            for val_to_set in [0, 1] if len(group_concepts) == 1 else [1]:
                ...
                _, _, change_y_preds, _, _ = self.cbm.cbm(
                    x,
    ...
    def _coop_step(
        ...
        if intervene_with_groups and self.cbm.cbm.use_concept_groups:
            ...
            if self.cbm.cbm.use_concept_groups and (not prior_distribution is None):
        ...
        if not self.cbm.cbm.use_concept_groups:
    ...
    def __call__(
        ...
        if self.group_based and self.cbm.cbm.use_concept_groups:
            n_groups = len(self.concept_group_map)
            prev_used_groups = torch.zeros((c.shape[0], n_groups)).to(c.device)
            for group_idx, (_, group_concepts) in enumerate(
                self.concept_group_map.items()
        ...
        if self.num_groups_intervened == 0:
            return mask, c
        _, _, y_preds, _, latent = self.cbm.cbm(
            x,
