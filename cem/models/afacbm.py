...
class AFAModel(pl.LightningModule):
    def __init__(self, cbm, config):
        ...
        try:
            self.env = gym.make("cem/models/AFAEnv-v0", cbm = self.cbm, ac_model = self.ac_model, env_config = afa_model_config)
        except:
            import pdb
            pdb.set_trace()
    ...
    #             next_obs = self.env.reset(seed=self.seed, options = options)[0]
    #             next_obs = {k: torch.tensor(v).to(intervention_idxs.device) for k, v in next_obs.items()}
    #             next_done = torch.zeros(1)

    #             for step in budget:
    #                 if len(obs) == 0:
    #                     for k, v in next_obs.items():
    #                         obs[k] = torch.zeros((batch_size * budget,) + v.shape)
                    
    #                 key = i * budget + step

    #                 for k in obs.keys():
    #                     obs[k][key] = next_obs[k]
    #                 dones[key] = next_done
    ...
    #                     next_obs, next_done = torch.tensor(nex  t_obs), done
